...
/** The browser firmware runner currently emulates an ATmega328P only. */
export function supportsAvr8js(type?: string): boolean {
  return type === 'ARDUINO_UNO'
    || type === 'ARDUINO_NANO'
...
/**
 * Resolve a board header label to the canonical Arduino-style pin token used
 * by the interpreter and the MNA board-driver model. The catalog contains
 * both Arduino aliases (D13/A0) and MCU aliases (GPIO13/GP0), so keeping this
 * normalization in one place prevents board-specific drift between the two
 * simulation paths.
 */
export function getBoardPinNumber(pin: { id: string; name: string }): string {
  ...
  const gpioMatch = label.match(/\bGPIO([0-9]{1,3})\b/i) || label.match(/\bGP([0-9]{1,3})\b/i);
...
/**
 * Return the voltage generated by a board power rail, or null when the pin is
 * an input rail (VIN/VSYS/BAT) or an ordinary GPIO. VCC/VDD/AVCC follow the
 * board's logic level; explicit 5V/3V3/VBUS labels retain their rail value.
 */
export function getBoardPowerVoltage(type: string, pin: { id: string; name: string }): number | null {
  const label = `${pin.name} ${pin.id}`.toUpperCase();
  if (/(^|\W)(VIN|VSYS|BAT)(\W|$)/.test(label)) return null;
  if (/(^|\W)(5V|VBUS|USB)(\W|$)/.test(label)) return 5;
  if (/(^|\W)(3V3|3\.3V|3V)(\W|$)/.test(label)) return 3.3;
  if (/(^|\W)(VCC|VDD|AVCC)(\W|$)/.test(label)) return getBoardLogicVoltage(type);
  return null;
}
