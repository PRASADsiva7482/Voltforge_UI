...
export default function BomPanel({ isOpen, onClose }: Props) {
  ...
  return (
      ...
      footer={
        bomItems.length > 0 ? (
          <div className="flex items-center justify-between w-full">
            <span className="text-[10px] text-surface-500 dark:text-surface-400">{bomItems.length} {t('types')} · {bomItems.reduce((s, i) => s + i.quantity, 0)} {t('total')}</span>
            <button onClick={exportCsv} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-volt-500/10 text-volt-600 text-[10px] font-bold hover:bg-volt-500/20 transition-colors dark:text-volt-400 cursor-pointer">
              <Download className="w-3 h-3" /> {t('Export CSV')}
            </button>
          </div>
        ) : undefined
      }
    >
      <div className="p-1">
        {bomItems.length === 0 ? (
          <p className="text-xs text-surface-500 text-center py-6">{t('Add components to canvas to generate BOM')}</p>
        ) : (
          <VfTable
            data={bomItems}
            ...
            columns={[
              ...
              },
              {
                ...
                render: (item) => (
                  <div>
                    <div className="text-surface-950 font-medium dark:text-white text-[10px]">{t(item.name)}</div>
                    <div className="text-surface-500 text-[9px]">{t(item.type.replace(/_/g, ' '))}</div>
                  </div>
                )
              },
              {
