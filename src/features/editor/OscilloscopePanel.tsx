...
export default function OscilloscopePanel({ isOpen, onClose, signalData = [] }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
      className="absolute bottom-64 right-4 w-80 glass rounded-2xl overflow-hidden z-30 shadow-2xl border border-white/10">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-surface-900/60">
        <div className="flex items-center gap-2"><Activity className="w-4 h-4 text-volt-400" /><span className="text-xs font-bold text-white">Oscilloscope</span></div>
        <div className="flex items-center gap-2">
          <button onClick={() => setPaused(!paused)} className="text-surface-400 hover:text-white">
      ...
      <div className="p-2">
        ...
        <div className="flex items-center justify-between mt-2 px-1">
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-surface-500">Time/div:</span>
            <select value={timeDiv} onChange={e => setTimeDiv(Number(e.target.value))}
              className="bg-white/5 text-[10px] text-white rounded px-1 py-0.5 border border-white/10">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[9px] text-surface-400">CH1: 0-5V</span>
          </div>
        </div>
      </div>
    </motion.div>
