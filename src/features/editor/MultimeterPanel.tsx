...
import { Activity, X, Zap, Gauge } from 'lucide-react';
...
export default function MultimeterPanel({ isOpen, onClose, voltage = 0, current = 0, resistance = 0 }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
      className="absolute top-16 right-4 w-56 glass rounded-2xl overflow-hidden z-30 shadow-2xl border border-white/10">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-surface-900/60">
        <div className="flex items-center gap-2"><Gauge className="w-4 h-4 text-volt-400" /><span className="text-xs font-bold text-white">Multimeter</span></div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-1">
          {(['V', 'A', 'Ω'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`py-2 rounded-lg text-xs font-bold transition-all ${mode === m ? 'bg-volt-500/20 text-volt-400 border border-volt-500/30' : 'bg-white/5 text-surface-400 hover:text-white border border-transparent'}`}>
              {m === 'V' ? '⚡ V' : m === 'A' ? '🔌 A' : '🔧 Ω'}
            </button>
          ))}
        </div>
