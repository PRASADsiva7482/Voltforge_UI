  ...
  if (isLoading) return (
    <div className="flex items-center justify-center h-screen bg-surface-950">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-2">VoltForge</h2>
        <p className="text-surface-400 text-sm">Loading your workspace...</p>
        <div className="mt-4 w-32 h-1 bg-surface-800 rounded-full mx-auto overflow-hidden"><div className="h-full bg-gradient-to-r from-volt-500 to-forge-500 rounded-full animate-[pulse_1.5s_ease-in-out_infinite]" style={{ width: '60%' }} /></div>
      </div>
    </div>
  );
  if (!isAuthenticated) return <div className="flex items-center justify-center h-screen bg-surface-950 text-white">Redirecting to login...</div>;
  return <>{children}</>;
}
